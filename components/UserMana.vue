        ...
        <div>
          ...
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px;display: flex;align-items: center">
            ...
            <el-switch v-model="user.enabled" active-text="启用" active-color="#13ce66"
              @change="enabledChange(user.enabled, user.id, index)" inactive-text="禁用" style="font-size: 12px">
            </el-switch>
          </div>
          <div style="text-align: left;color:#20a0ff;font-size: 12px;margin-top: 13px">
            <span>用户角色:</span>
            ...
            <el-popover placement="right" title="角色列表" width="200" :key="index+''+user.id"
              ...
              <el-select v-model="roles" :key="user.id" multiple placeholder="请选择" size="mini">
                <el-option v-for="(item) in allRoles" :key="item.id" :label="item.name"
                  :value="item.id">
                </el-option>
                <el-button type="button" round="8px" size="mini" style="margin-left: 130px" @click="saveRoles(user.id, index)">
                    修改
                </el-button>
              </el-select>
...
export default {
  ...
  methods: {
    saveRoles(id, index) {
      ...
      putRequest("/admin/user/updateUserRoles", { rids: this.roles, id: id }).then(resp => {
    ...
    deleteUser(id) {
      ...
      this.$confirm('删除该用户, 是否继续?', '提示', {
        ...
        _this.loading = true;
        console.log(id)
        deleteRequest("/admin/user/deleteUserById/" + id).then(resp => {
          console.log(resp.data)
          if (resp.data.status == 200 && resp.data.msg == 'success') {
    ...
    enabledChange(enabled, id, index) {
      ...
      var _this = this;
      _this.cardloading.splice(index, 1, true)
      putRequest("/admin/user/updateUserEnabled", { enabled: enabled, id: id }).then(resp => {
        if (resp.status !== 200) {
          _this.$message({ type: 'error', message: '更新失败!' })
          _this.loadOneUserById(id, index);
          return;
    ...
    loadRoles(index) {
      ...
      getRequest("/admin/user/getAllRoles").then(resp => {
    ...
    loadOneUserById(id, index) {
      var _this = this;
      getRequest("/admin/user/getUserById/" + id).then(resp => {
        _this.cardloading.splice(index, 1, false)
        if (resp.status == 200) {
    ...
    loadUserList() {
      var _this = this;
      getRequest("/admin/user/getAllUserByNickname" + "?nickname=" + this.keywords).then(resp => {
        _this.loading = false;
        if (resp.status == 200) {

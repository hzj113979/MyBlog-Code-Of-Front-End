...
export default {
  ...
  mounted: function () {
    var _this = this;
    getRequest("/user/currentUserEmail").then(resp => {
      if (resp.status == 200) {
        _this.emailValidateForm.email = resp.data.msg;
      }
    });
  },
  methods: {
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          _this.loading = true;
          putRequest("/user/updateUserEmail", { email: _this.emailValidateForm.email }).then(resp => {
            _this.loading = false;
            if (resp.status == 200) {
